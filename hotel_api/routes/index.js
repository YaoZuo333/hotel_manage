...
//修改密码
router.post('/api/change_pwd',(req,res)=>{
    // 1. 获取数据
    ...
    let sqlStr = "UPDATE hotel_user SET user_psw = ? WHERE user_Id = " + id;
    let strParams = [pwd];
    console.log(id);
...
/*获取客户信息*/
router.get('/api/get_member_info',(req,res)=>{
    let sqlStr = "SELECT * FROM hotel_user WHERE user_position = '会员'";
...
router.post('/api/search_member',(req,res)=>{
    ...
    if (!name){
        res.json({err_code:0,message:'客户姓名错误'});
        return
    }
    let sqlStr = "SELECT * FROM hotel_user WHERE user_name = '"+name+"' AND user_position = '会员'" ;
    conn.query(sqlStr,(err,results,fields)=>{
        if (err){
            res.json({err_code:0,message:'数据库查询失败',err})
        } else {
...
/*添加客户信息*/
router.post('/api/add_member_info',(req,res)=>{
    let phone = req.body.user_phone;
    let userID = req.body.user_phone;
    let psd = req.body.user_Psw;
    ...
    conn.query(sqlStr,(err,results,fields)=>{
        if (err){
            res.json({err_code:100,message:'查询数据库失败'+err});
        } else {
            results = JSON.parse(JSON.stringify(results))[0];
            if (results){
                res.json({err_code:101,message:'该手机已经注册'});
            }else {
                let addSql = "INSERT INTO hotel_user(user_Id,user_name, user_phone,user_psw,user_position,user_age,user_sex,user_address) VALUES (?, ?, ?, ?, ?, ?, ?,?)";
                const addparmas = [userID,name,phone,psd,'会员',age,sex,address];
                console.log(addparmas);
                conn.query(addSql,addparmas,(err,results,fields)=>{
                    if (err){
                        //console.log(err);
                        res.json({err_code:102,message:'查询数据库失败'+err});
                    }else {
                        results = JSON.parse(JSON.stringify(results));
                        res.json({success_code:200,message:results});
                    }
...
/*移除客户信息*/
router.post('/api/remove_member',(req,res)=>{
    // 1. 获取数据
    const removeLis = req.body.removeArr || '';
    let delSql = "DELETE FROM hotel_user WHERE user_Id in (?)";
    for (let i=0;i<removeLis.length;i++){
        conn.query(delSql,removeLis[i],(err,results,fields)=>{
            if (err){
                res.json({err_code:0,message:'查询数据库失败'+err});
            ...
            if (i == removeLis.length-1){
                results = JSON.parse(JSON.stringify(results));
                console.log(results);
                res.json({success_code:200,message:'移除客户信息成功！'})
            }
...
/*修改客户信息*/
router.post('/api/change_member',(req,res)=>{
    // 1. 获取数据
    const id = req.body.changeInfo.user_Id;
    const name = req.body.changeInfo.user_name || '';
    const sex = req.body.changeInfo.user_sex || '';
    const phone = req.body.changeInfo.user_phone || '';
    const address = req.body.changeInfo.user_address || '';
    const age = req.body.changeInfo.user_age || '';
    //console.log("000000000000");
    //console.log(id,name,department,age);
    ...
    let sqlStr = "UPDATE hotel_user SET user_name = ? , user_sex = ?, user_phone = ?, user_address = ? ,user_age = ?  WHERE user_id = " + id;
    let strParams = [name, sex, phone,
        address,age];
    console.log(id);
    conn.query(sqlStr,strParams,(err,results,fields)=>{
        if (err){
            ...
            results = JSON.parse(JSON.stringify(results));
            console.log(results);
            res.json({success_code:200,message:'修改客户信息成功！'})
        }
    })
...
router.post('/api/add_order',(req,res)=>{
    ...
    let addSql = "INSERT INTO hotel_order(order_type,order_startdate,order_enddate,u_id,order_price,order_number,order_total,order_state,order_others,order_peoples) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
    const addparmas = [type,start,end,userId,price,number,total,"未处理", others,peoples];
    console.log(addparmas);
    conn.query(addSql,addparmas,(err,results,fields)=>{
        if (err){
            res.json({err_code:0,message:'查询数据库失败'+err});
...
/*特定时间段数据*/
router.get('/api/special_time_data',(req,res)=>{
    const start = req.query.start;
    const end = req.query.end;
    let sqlStr = "SELECT sum(order_total) total_price ,COUNT(order_number) total_rooms,SUM(order_peoples) total_peoples from hotel_order WHERE order_state = '已入住' AND order_startdate between '"+start+"' and '"+end+"'";
